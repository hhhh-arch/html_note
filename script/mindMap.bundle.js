...
`).forEach((c, l) => {
  ...
  function readDOMChange(view2, from2, to2, typeOver, addedNodes) {
    ...
    let mkTr = (base2) => {
      ...
      if (parse.sel) {
        ...
        if (sel2 && !(chrome2 && view2.composing && sel2.empty && (change.start != change.endB || view2.input.lastChromeDelete < Date.now() - 100) && (sel2.head == chFrom || sel2.head == tr.mapping.map(chTo) - 1) || ie2 && sel2.empty && sel2.head == chFrom))
  ...
  function keyName(event) {
    var ignoreKey = mac3 && event.metaKey && event.shiftKey && !event.ctrlKey && !event.altKey || ie3 && event.shiftKey && event.key && event.key.length == 1 || event.key == "Unidentified";
  ...
  return __toCommonJS(mindMap_exports);
})();
